	...
	<div class="container table-hover">
      <div class="row">
        <div class="col-md-6">분양 정보</div>
					...
					<div class="col-md-2" style="text-align:right;"><a href="sellregister.do"><input class="btn btn-outline-primary" type="button" value="등록"></a></div>
				</div>	
				...
				<% for(SellDTO s : sList) { %>
				<div class="row">
					<div class="col-md-2"><%=s.getSellSeqNo() %></div>
					<div class="col-md-4"><a href="/selldetail.do?sellSeqNo=<%=s.getSellSeqNo() %>&pageCount=<%=paging.getPage_count()%>&pageNum=<%=paging.getPage_num()%>&keyword=<%=keyword%>"><%=s.getTitle() %></a></div>
					<div class="col-md-3"><%=s.getUserName() %></div>
					<div class="col-md-3"><%=s.getChgDt() %></div>
				</div>
				<% } %>
